...
function MapQueue:__init(args)
    getter_setter(self, "course_enum")
    self.course_enum = args.course_enum
    self.name = args.map
    self.difficulty = args.difficulty
    self.countdown_active = false
    self.countdown = LobbyManager.countdown_max

    self.players = {}
end

function MapQueue:GetCountdownSyncInfo()
    return {
        course_enum = self.course_enum,
        active = self.countdown_active,
        current_time = self.countdown,
        max_time = LobbyManager.countdown_max
    }
end
